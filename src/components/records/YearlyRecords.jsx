...
const YearlyRecords = ({ isDarkMode, employees }) => {
  ...
  const yearlyData = useMemo(() => {
    const monthlyTotals = Array.from({ length: 12 }).map((_, month) => {
      const monthData = employees.map(emp => calculateMonthlyPayroll(emp, year, month));
      return monthData.reduce((acc, record) => ({
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="min-w-full">
          <thead className={`${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            <tr>
              <th className={`px-4 py-3 text-left text-xs font-semibold ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase`}>Month</th>
              <th className={`px-4 py-3 text-right text-xs font-semibold ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase`}>Gross Salary</th>
              <th className={`px-4 py-3 text-right text-xs font-semibold ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} uppercase`}>Deductions</th>
            ...
            {yearlyData.map((monthData, idx) => (
              <tr key={idx} className={`${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
                ...
                <td className={`px-4 py-3 text-right font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  PKR {monthData.gross.toLocaleString()}
                </td>
                <td className="px-4 py-3 text-right text-red-600 font-medium">
                  PKR {monthData.deductions.toLocaleString()}
                </td>
                <td className="px-4 py-3 text-right text-indigo-600 font-bold">
                  PKR {monthData.net.toLocaleString()}
                </td>
              </tr>
            ))}
            <tr className={`${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'} font-bold`}>
              <td className={`px-4 py-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>ANNUAL TOTALS</td>
              <td className={`px-4 py-3 text-right ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                PKR {yearlyTotals.gross.toLocaleString()}
              </td>
              <td className="px-4 py-3 text-right text-red-600">
                PKR {yearlyTotals.deductions.toLocaleString()}
              </td>
              <td className="px-4 py-3 text-right text-indigo-600">
                PKR {yearlyTotals.net.toLocaleString()}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div className="md:hidden space-y-3">
          ...
          <div key={idx} className={`${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg p-4`}>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className={isDarkMode ? 'text-gray-400' : 'text-gray-600'}>Gross:</span>
                <span className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  PKR {monthData.gross.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between">
                <span className={isDarkMode ? 'text-gray-400' : 'text-gray-600'}>Deductions:</span>
        ...
        <div className={`${isDarkMode ? 'bg-gray-900' : 'bg-gray-200'} rounded-lg p-4`}>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span>Gross:</span>
              <span className="font-semibold">PKR {yearlyTotals.gross.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
